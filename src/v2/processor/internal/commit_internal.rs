...
use crate::{
    ...
    v2_require_eq, v2_require_eq_keys, v2_require_ge, v2_require_owned_by,
    v2_require_signer,
};

/// Arguments for the commit state internal function
pub(crate) struct CommitInternalArgs<'a> {
...
#[inline(always)]
pub(crate) fn process_commit_internal(
    args: CommitInternalArgs,
) -> Result<(), ProgramError> {
    ...
    let state_data = unsafe { args.delegation_state.borrow_unchecked_mut() };
    ...
    let state_view =
        unsafe { &mut *(header.as_mut_ptr() as *mut DelegationStateHeader) };
